...
use ADT\Ratchet\Router\IRouter;
 ...
 *
 */
class ControllerApplication extends \Nette\Object implements MessageComponentInterface {
	...
	public function addController($controller) {
		$this->controllers[$controller->getReflection()->getShortName()] = $controller;
	}
	...
	public function onMessage(ConnectionInterface $from, $msg)
	{
		...
		if (!$request instanceof Request)
			throw new InvalidArgumentException('Route must return ADT\Ratchet\Request.');

		$className = $request->getControl();
		if (!$className || !class_exists($className))
			throw new BadRequestException('Control class "' . $className . '" not found.');

		if (!is_subclass_of($className, 'ADT\Ratchet\UI\Control'))
			throw new BadRequestException($className . ' must be subclass of ADT\Ratchet\UI\Control.');
		...
		/** @var \ADT\Ratchet\UI\Control $control */
